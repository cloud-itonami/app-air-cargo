(ns air-cargo.route-test
  (:require [clojure.test :refer [deftest is testing]]
            [clojure.string :as str]
            [air-cargo.route :as route]
            [air-cargo.view :as view]))
...
(deftest dispatch-xrpc
  (testing "nsid はそのまま渡す"
    (is (= {:action :xrpc :nsid "com.etzhayyim.apps.airCargo.issueAirWaybill"}
           (route/dispatch "POST" "/xrpc/com.etzhayyim.apps.airCargo.issueAirWaybill"))))
...
(deftest capability-shaping
  (testing "短い名前には NSID prefix を足す —— wrangler の APP_CAPABILITIES の 3 つ"
    (is (= ["com.etzhayyim.apps.airCargo.createCargoBooking"
            "com.etzhayyim.apps.airCargo.issueAirWaybill"
            "com.etzhayyim.apps.airCargo.acceptCargo"]
           (route/capability-nsids ["createCargoBooking" "issueAirWaybill" "acceptCargo"]))))
  (testing "既に完全修飾なものは二重に付けない"
    (is (= ["com.etzhayyim.apps.airCargo.assignUld"]
           (route/capability-nsids ["com.etzhayyim.apps.airCargo.assignUld"]))))
...
(deftest page-shows-the-real-data
  (testing "ページは渡された値から描く。0 も [] も焼かない（docs/adr/0001 の欠陥）"
    (let [methods (route/capability-nsids ["createCargoBooking" "issueAirWaybill" "acceptCargo"])
...
(deftest page-shows-what-it-is-given-not-a-baked-table
  (testing "同じ view に別の値を渡せば別の結果になる。ページが引数を無視して
    ...
    (let [a (view/render {:css "" :routes [{:route/path "/only-a" :route/method :get
                          ...
                          :methods ["com.example.a"] :vars [:VAR_A]
          ...
          b (view/render {:css "" :routes [{:route/path "/only-b" :route/method :post
                          ...
                          :methods ["com.example.b"] :vars [:VAR_B]
