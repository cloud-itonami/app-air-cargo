(ns air-cargo.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `air-cargo.route/dispatch`
  が決め、ページの中身は `air-cargo.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。

  wrangler.jsonc の `main` は `dist/worker.js` を指し、それはこの名前空間を
  コンパイルしたものである。移行前は `svelte/.svelte-kit/cloudflare/_worker.js`
  —— tree に存在しないビルド出力 —— を指していて、読み手が開く `src/app.ts` は
  どの config からも参照されていなかった（docs/adr/0001）。

  `aget` を使うのは `:advanced-optimization` 下で env のキーが潰れないため
  （先例 `listingops.edge.worker` と同じ約束）。"
  (:require [air-cargo.route :as route]
