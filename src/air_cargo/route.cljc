(ns air-cargo.route
  ...
  build, or a network. `air-cargo.worker` is the only namespace that touches
...
(def nsid-prefix
  "air-cargo の XRPC 名前空間。移行前は 3 箇所が別々に宣言していた:
  `kotodama.jsonld` の `triggers.subscribeRepos.collections`
  (`com.etzhayyim.apps.airCargo.operationalEvent`)、撤去した `src/app.ts` の
  `NSID_PREFIX`、そして `wrangler.jsonc` の `APP_CAPABILITIES` が並べる 3 つの
  短いメソッド名。**この prefix は表示のためだけに使う** —— 中継そのものは
  deploy されていた SvelteKit の route と同じく prefix を検査しない。"
  "com.etzhayyim.apps.airCargo.")

(def actor-did
  "`kotodama.jsonld` の `@id`。ここに写しているのは `/health` が名乗るためで、
  正本はあちら側。DID document は取得できない —— `did:web` の解決には
  `https://air-cargo.etzhayyim.com/.well-known/did.json` が要るが、そのホストは
  NXDOMAIN である（README の DNS 表）。"
  "did:web:air-cargo.etzhayyim.com")

(def default-nanoid
  "撤去した `src/app.ts` が `env.APP_NANOID ?? \"a1rcarg0\"` と書いていた既定値。
...
(def routes
  ...
  docs/adr/0001 records was a page that said `Routes 0`, `routes []` and
  `vars []` beside a wrangler.jsonc declaring two routes and eight vars."
...
(defn capability-nsids
  ...
  decode（JSON 文字列 → seq）は `air-cargo.worker` の側にある。**ここは形だけを
  決める** —— すでに完全修飾なものは二重に prefix を付けない（宣言側が長い名前に
  変わってもページが壊れない）。"
  [names]
  (into []
        (comp (filter string?)
